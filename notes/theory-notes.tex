...
% XXX: add if place left
%   instance vs model learning \cite{geron_hands-machine_2017}
%   make more statistical
%   Bias-Variance tradeoff

% negative log likelihood for Bernoulli Distribution binary classification as another example
% https://medium.com/@bhardwajprakarsh/negative-log-likelihood-loss-why-do-we-use-it-for-binary-classification-7625f9e3c944

It is derived by \ldots % MSE
% introduce Maximum Likelihood Estimation
% p. 130: maximum likelihood -> normal distribution -> MSE
% p. 128f.: maximum likelihood can be rescaled to cross entropy (not changing minimum) which in
%           turn can be rescaled to KLD -> reduce dissimilarity between empirical distribution
%           defined by training set and model distribution
% p. 129: cross entropy refers to any negative log-likelihood between empirical and model distribution
...
\textbf{Michigan lecture, CNN and architectures}
