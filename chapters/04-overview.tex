...
regression based (see
Figure~\ref{fig:STD-pipelines})~\citep{long_scene_2021,sheng_centripetaltext_2021,liu_accurate_2020}.
For \ac{STR} two main categories of approaches can be identified: segmentation based and segmentation
less~\citep{chen_text_2021}.
Segmentation less approaches can again be divided into CTC based and Encoder-Decoder based
...
For segmentation based \ac{STR} the approach introduced by~\cite{liao_scene_2018} (see
Figure~\ref{fig:STR-segbased-architecture}) is used.
\begin{figure}[h]
    \centering
    \include{img/STR-seg-based-Liao-Scene-2018.png}
    \caption[Segmentation based STR architecture]{%
        Example for a segmentation based STR
        architecture~\citep{liao_scene_2018}\label{fig:STR-segbased-architecture}
    }
\end{figure}
The architecture is similar to~\cite{deng_pixellink_2018} which introduced a segmentation based
\ac{STR} approach.

Segmentation less~\cite{liao_scene_2018}
\begin{itemize}
