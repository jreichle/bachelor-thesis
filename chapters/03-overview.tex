...
For object detection inspired \ac{STD} there are two methods: one-stage and
...
\end{figure}
One-stage approaches are modelled after~\cite{liu_ssd_2016}, \ac{SSD} or~\cite{redmon_you_2016},
...
Of all detections which overlap (\ac{IOU}) more than a threshhold
...
Instead of detecting whole \acp{BB}, the networks predicts both sub-text components and links at
...
mechanism in the sequence modelling and prediciton stage.
...
For \ac{EnDe}-based \ac{STR} there are two main categories: \ac{CTC}-based and
...
networks~\citep{jaderberg_spatial_2015} show great improvement for \ac{STR}
...
Sequence modelling is used to generate contextual information from a sequence of characters rather
...
This acts acts as a connector of visual representation and the sequence representation
...
\end{figure}
Deep \ac{BiLSTM} are computation expensive and are therefore often replaced with less expensive
...
The prediction layer uses the sequential, context enriched data to predict a character sequence.
The example approach transciption uses conditional probibilities to convert the per-frame features
encoded by the deep \ac{BiLSTM} into the label sequence which is the text instance
...
For an attention based example, the approaches introduced by~\cite{shi_robust_2016,ghosh_visual_2017}
are used.
For attention two approaches are explained as they are semantically different in
that~\citep{shi_robust_2016} employs a sequence attention mechanism while~\cite{ghosh_visual_2017}
employs a visual attention mechanism.
Figure~\ref{fig:STR-sequence-attention} shows the architecture of the sequence attention mechanism
based approach~\citep{shi_robust_2016}.
\begin{figure}[h]
    \centering
    \includegraphics[width=0.5\textwidth]{img/STR-encdec-attention-Shi-Robust-2016.png}
...
\end{figure}
After the feature extraction stage, the last convolutional feature map is used to generate feature
...
The sequence is then used by the attention mechanism which is explained
in~\citep{chorowski_attention-based_2015}:
\begin{equation}\label{eq:sequential-attention-multi}
    \textbf{g}_t=\boldsymbol{\alpha}_t \otimes \textbf{h}_t
\end{equation}
\begin{equation}\label{eq:sequential-attention-weights}
    \boldsymbol{\alpha}_i=\sfmx(\Phi(\textbf{s}_{t-1},\boldsymbol{\alpha}_{t-1},\textbf{h}_t))
\end{equation}
\begin{equation}\label{eq:sequential-attention-scoring}
    \Phi=A\tanh(W\textbf{s}_{t-1}+V\h_t+U(F\boldsymbol{\alpha}_t-1)+b)
\end{equation}
The important equation is~\ref{eq:sequential-attention-multi}.
Here the attention weights $\boldsymbol{\alpha}_t$ are used to determine which part of the input
$\h_t$ is important~\citep{shi_robust_2016}.
This is possible because the non-negative \sfmx\ has outputs between 0 and 1.
...
The Gated Recurrent Units output use \sfmx\ to output a probability distribution over the possible
characters (alphanumeric english + `end-of-sequence' token)~\citep{shi_robust_2016}.
The prediction/transciption stage is similar to the next approach and will be explained after the
visual attention mechanism.

The approach from~\cite{ghosh_visual_2017} is inspired by~\cite{bahdanau_neural_2016,xu_show_2016}:
It uses a \ac{CNN} for spatial encoding of the input image which can then be used.
...
The feature map is separated into feature vectors $\x_i$ which represent different parts
of the image.
The mechanism uses a weighted combination of the \sfmx\ activation ($\boldsymbol{\beta}_{t,i}$) of
...
\begin{equation}\label{eq:attention-weighted-combination}
    \hat{\textbf{z}}_t=\sum_{i=1}^{K}\beta_{t,i}\cdot \x_i
\end{equation}
\begin{equation}\label{eq:attention-gate-values}
    \boldsymbol{\beta}_{t}=\sfmx(\Phi(\x_i,\h_{t-1}))
\end{equation}
In contrast to the previous approach, the attention mechanism considers the whole image while the
previous one considers one time step with respect to the previous
step~\citep{shi_robust_2016,ghosh_visual_2017}.
The prediction stage goes as follows:
Like with the \ac{CTC}-based approach, the networks output vectors at each time step represent a
...
Note that for the 2-step modular approaches can still be adjusted to better fit
together~\citep{liao_textboxes_2017}.
The 2-stage approach that is used as an example was introduced by~\cite{busta_deep_2017}.
...
\begin{figure}[ht]
    \centering\scriptsize
    \subfigure[
        \scriptsize Robust neighbor adjusted 2d attention
