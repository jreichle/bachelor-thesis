...
% FIXME: show difference between axis-aligned, rotated and quadrilateral
...
% FIXME: differentiate between: semantic and instance segmentation (detection is instance segmentation)
%                   what is meant by segmentation based for taxonomy
% FIXME: add to different steps: what are other popular choices e.g. backbone ResNet and FPN
% FIXME: challenging properties
...
% FIXME: introduce masking, preferably from Mask RCNN
...
% FIXME: emphasize that sequence features are basically 1d
% FIXME: \cite{cong_comparative_2019}: other ways  to encode 1d: recusive CNN, self attention
%   after CNN
...
\end{figure}
% FIXME: get into forward backward algorithm (graves_connectionist_2006)
...
% FIXME: The \ac{CNN} architecture introduced by~\cite{jaderberg_deep_2015} is used.
...
% FIXME: attention mechanism can also be understood as part of decoder~\cite{shi_aster_2019}
...
% FIXME: performance - efficiency tradeoff for feature extractor
...
% FIXME: two stage approach is very similar to STD two stage -> get ROI and then refine after pooling
...
\begin{table}[h]
    \centering\scriptsize
    \begin{tabular}{p{.09\textwidth}p{.16\textwidth}p{.20\textwidth}p{.065\textwidth}
            p{0.29\textwidth}p{0.04\textwidth}}
...
\begin{figure}[ht]
    ...
    ]{\includegraphics[width=0.40\textwidth]{img/STD-representation-curved-binary-Liao-Real-2019.png}}
    \subfigure[
...
\begin{table}[h]
    \centering\scriptsize
    \begin{tabular}{p{.09\textwidth}p{.165\textwidth}p{.20\textwidth}p{.065\textwidth}
            p{0.29\textwidth}p{0.04\textwidth}}
...
\end{table}
The only approach~\citep{liao_scene_2018} that is not assigned to attention based actually also
contains attention.
...
\begin{table}[h]
    \centering\scriptsize
    \begin{tabular}{p{.09\textwidth}p{.20\textwidth}p{0.07\textwidth}p{0.07\textwidth}
            p{0.37\textwidth}p{0.04\textwidth}}
