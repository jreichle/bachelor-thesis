...
comparison among approaches for all aforementioned tasks categories~\citep{long_scene_2021}.
...
\begin{table}[h]\label{tb:confusionMatrix}
    \centering
    \makebox[\textwidth][c]{\begin{tabular}{cccc}
        \toprule
                  ...
                  & & positive & negative \\
        \midrule
        Prediction & \multicolumn{1}{c|}{\makecell{positive\\negative}}
...
\end{equation}
The difference manifests itself in the way the values of the confusion matrix are
calculated~\citep{long_scene_2021}.
...
% difference in training and testing/using -> threshhold - highest value
...
Per~\cite{su_relationship_2015}  \ac{AP} can be considered the area under the precision-versus-recall
curve.
\fone\ on the other hand only considers singular instances on the curve~\citep{long_scene_2021}.
% STR, STS
For \ac{STR} and \ac{STS} the evaluation can be based on character-level or word-level.
Note that for \ac{STS} the prediction has to be matched to the ground truth like for
\ac{STD}~\citep{long_scene_2021}.
Also compared with loss function
\cite{yang_learning_2021} uses hmean/ap
