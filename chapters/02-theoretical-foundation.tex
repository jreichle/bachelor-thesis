...
%% evaluation metrics
% FIXME: properties and pros for different metrics
% FIXME: table with task, metric, properties/why it's used
...
Note that the tradeoff between false positives and fales negaives manifests itself in the
...
% difference in training \& testing and using -> threshhold - highest value
...
\fone\ on the other hand only considers singular instances on that curve~\citep{long_scene_2021}.
% STR
...
\end{equation}
An example for a character-based metric would be \ac{NED} (see Equation~\ref{eq:ned}).
...
For comparing words that have been matched, e.g. \ac{NED} is used~\citep{chen_text_2021}.
The competitions~\cite{karatzas_icdar_2013,karatzas_icdar_2015} define two different tasks for
\ac{STS} systems.
Word spotting refers to the task of only finding words of a predefined vocabulary, while the
end-to-end setting specifies that all text is to be found and recognized.
For end-to-end the main evaluation protocols that are used include Precision, Recall, \fone\ and
\ac{AED}~\citep{chen_text_2021}.
Note that \ac{NED} is used for the Precision and Recall scores and \ac{AED} is the sum of
\ac{NED} values devided by the number of pictures~\citep{chen_text_2021}.
% FIXME: how?
...
\begin{table}[ht]
    ...
    \begin{tabular}{p{.2\textwidth}p{.05\textwidth}p{.05\textwidth}p{.15\textwidth}p{.3\textwidth}}
        ...
        Total-Text & \checkmark& \checkmark&Curved& Polygon labeles \\
        IIIT 5K-Word & &\checkmark&Horizontal&Cropped \\
        \bottomrule
    \end{tabular}
