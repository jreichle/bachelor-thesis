...
(e.g.\ regression --- $f:\R^n \mapsto \R^l$, classification ---
$f:\R^n \mapsto \{1,\ldots,k\}$) to obtaining a different representation for the data that
...
The number of layers $d$ is refered to as depths whereas the dimensionality of those layers is called
the width $w$~\citep{goodfellow_deep_2016}.
...
For simplification both $\w,b$ are refered to as $\T$ ($\T_:,n$ equals 1 for the bias term).
